...
describe Dor::Contentable do
  ...
  describe "#copy_file_resources" do
    ...
    context "secondary files names that collide with primary file names" do

      it "appends sequence number to filenames when checksums don't match" do
        src1.contentMetadata.content = <<-XML
        <?xml version="1.0"?>
        <contentMetadata objectId="ab123cd0002" type="map">
          <resource id="ab123cd0002_1" sequence="1" type="image">
        ...
        expect(merged_cm.xpath("//resource[@sequence = '2']").size).to eq(1)
        expect(merged_cm.at_xpath("//resource[@sequence = '2']/@id").value).to  eq('image_2')
        expect(merged_cm.at_xpath("//resource[@sequence = '2']/file/@id").value).to eq('image_a_2.jp2')
        expect(merged_cm.at_xpath("//resource[@sequence = '3']/@id").value).to  eq('image_3')
        expect(merged_cm.at_xpath("//resource[@sequence = '3']/file/@id").value).to eq('image_c.jp2')
      end

      it "raises an exception if checksums match" do
        src1.contentMetadata.content = <<-XML
        <?xml version="1.0"?>
        <contentMetadata objectId="ab123cd0002" type="map">
          <resource id="ab123cd0002_1" sequence="1" type="image">
        ...
        expect{primary.copy_file_resources([src1_pid])}.to raise_error(Dor::Exception,
          "Exact file (name and checksum) 'image_a.jp2' from secondary object druid:ab123cd0002 already exist in primary object: druid:ab123cd0001")
      end

      it "generates checksums then compares if checksums do not exist" do
        pending
      end
