...
describe Dor::CleanupResetService do
  ...
  # clean_renamed_workspace
  
 # ck111ck 
    
 context "cleanup_by_renamed_druid" do
   before(:each) do
     
   end
   
  it "should remove the renamed druid tree from workspace and renamed bag from export" do
    ...
    Dor::CleanupResetService.cleanup_by_renamed_druid(druid)
 ...
 #cleanup_renamed_workspace_content
...
context "cleanup_renamed_workspace_content" do
  it "should remove the renamed directory in workspace" do
    ...
    last_version = 1
    expect(File.exists?(base_druid_dir+"_v1")).to eq true
    Dor::CleanupResetService.cleanup_renamed_workspace_content(druid, last_version, @workspace_root_pathname.to_s)
    expect(File.exists?(base_druid_dir+"_v1")).to eq false
  end
  
  it "should remove the renamed directory and keep the open version" do
  ...
  it "should remove 1 version with immediate ancestor" do
    ...
    Dor::CleanupResetService.cleanup_renamed_workspace_content(druid_1, last_version, @workspace_root_pathname.to_s)
 ...
 context "get_renamed_dirctories_list" do
   before(:each) do
  ...
  it "should get one renamed directory from workspace" do
    druid = "druid:#{@druid_1v}"
    base_druid_tree = DruidTools::Druid.new(druid, @workspace_root_pathname.to_s)
    last_version = 1
    dir_list = Dor::CleanupResetService.get_renamed_dir_list(last_version, base_druid_tree.path)
  ...
  it "should get two renamed directories from workspace" do
  ...
  it "should get one renamed directory from workspace and avoid the open version" do
  ...
  ## cc111ce1111 - 2 dir opened and versioned and 2 tar opened and renamed
  ## 
 context "cleanup_renamed_export" do
   before(:each) do
   ...
   it "should remove both bag tar and directory" do
     ...
     Dor::CleanupResetService.cleanup_renamed_export(druid,1)
     expect(File.exists?(base_bag_dir)).to eq false
     expect(File.exists?(base_bag_dir+".tar")).to eq false
   end
 end
 
 context "get_renamed_bag_dir_list" do
   before(:each) do
   ...
   it "should read the bag directory with 1 version" do
     ...
     dir_list =  Dor::CleanupResetService.get_renamed_bag_dir_list(1,base_bag_dir)
   ...
   it "should return a list of bag directories with 2 versions" do
     ...
     dir_list =  Dor::CleanupResetService.get_renamed_bag_dir_list(3,base_bag_dir)
 ...
 context "get_renamed_bag_tar_list" do
   before(:each) do
     @druid_1v = "cc111ca1111"
     @druid_2v = "cc111cb1111"
     @druid_1_1v = "cc111cc1111"
   ...
   it "should return a list of tars with 1 version" do
     druid = "druid:#{@druid_1v}"
     base_bag_dir = "#{@export_pathname}/#{@druid_1v}"
     tar_list =  Dor::CleanupResetService.get_renamed_bag_tar_list(1,base_bag_dir)
   ...
   it "should return a list of tars with 2 versions" do
     druid = "druid:#{@druid_2v}"
     base_bag_dir = "#{@export_pathname}/#{@druid_2v}"
     tar_list =  Dor::CleanupResetService.get_renamed_bag_tar_list(3,base_bag_dir)
