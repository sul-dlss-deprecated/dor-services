...
describe Dor::SearchService do
  ...
  context 'indexing functions' do
    before :each do
      stub_request(:get, 'http://fedora.edu/gsearch/rest/?action=fromPid&operation=updateIndex&value=druid:bb110sm8219').to_return(:body => 'OK')
      stub_request(:get, 'http://fedora.edu/gsearch/rest/?action=fromPid&operation=updateIndex&value=druid:bb110sm8210').to_return(:body => 'OK')
    end

    it 'should report an index version' do
      expect(Dor::SearchService.index_version).to match(/\d+\.\d+\.\d+/)
    end

    it 'should reindex PIDs' do
      result = Dor::SearchService.reindex('druid:bb110sm8219', 'druid:bb110sm8210') do |group|
        expect(group).to eq(['druid:bb110sm8219', 'druid:bb110sm8210'])
      end
      expect(result).to eq(['druid:bb110sm8219', 'druid:bb110sm8210'])
    end
  end
  ...
  context '.query' do
    before :each do
      ...
      stub_request(:get, "http://solr.edu/solrizer/select?fl=id&q=#{Solrizer.solr_name('dor_id', :stored_searchable)}%3A%22barcode%3A9191919191%22&rows=14&start=0&wt=ruby").to_return(:body => ruby_responses[0])
      stub_request(:get, "http://solr.edu/solrizer/select?fl=id&q=#{Solrizer.solr_name('dor_id', :stored_searchable)}%3A%22barcode%3A9191919191%22&rows=14&start=14&wt=ruby").to_return(:body => ruby_responses[1])
      stub_request(:get, "http://solr.edu/solrizer/select?fl=id&q=#{Solrizer.solr_name('dor_id', :stored_searchable)}%3A%22barcode%3A9191919191%22&rows=14&start=28&wt=ruby").to_return(:body => ruby_responses[2])
    end
  ...
  context '.query_by_id' do
    ...
    it 'should look up an object based on any of its IDs' do
      ...
      stub_request(:get, "http://solr.edu/solrizer/select?fl=id&q=#{Solrizer.solr_name('identifier', :stored_searchable)}%3A%22barcode%3A9191919191%22&rows=1000&start=0&wt=ruby").to_return(:body => ruby_responses[0])
      stub_request(:get, "http://solr.edu/solrizer/select?fl=id&q=#{Solrizer.solr_name('identifier', :stored_searchable)}%3A%22barcode%3A9191919191%22&rows=1000&start=1000&wt=ruby").to_return(:body => ruby_responses[1])
    ...
    it 'should perform a solr search' do
      ...
      stub_request(:get, 'http://solr.edu/gsearch/select?facet=on&facet.field=isMemberOfCollection_id_facet&facet.field=project_tag_facet&q=object_type_field:item&rows=2&wt=json').to_return(:body => json_response)
      real_response = Dor::SearchService.gsearch :q => 'object_type_field:item', :rows => 2, :facet => 'on', :'facet.field' => %w(isMemberOfCollection_id_facet project_tag_facet)
      expect(real_response).to eq(JSON.parse(json_response))
    end
