...
RSpec.describe 'Adding release nodes', :vcr do
  ...
  describe 'Adding tags and workflows' do
    ...
    it 'releases an item with one release tag supplied' do
    ...
    it 'releases an item with multiple release tags supplied' do
      allow(item).to receive(:save).and_return(true) # stud out the true in that it we lack a connection to solr
      expect(item).to receive(:create_workflow).with('releaseWF') # Make sure releaseWF is called
  ...
  describe 'valid_release_attributes' do
    ...
    it 'raises an error when :who, :to, :what are missing or are not strings' do
    ...
    it 'does not raise an error when :what is self or collection' do
    ...
    it 'adds a tag when all attributes are properly provided, and drop the invalid <tag> attribute' do
    ...
    it 'fails to add a release node when there is an attribute error' do
      VCR.use_cassette('simple_release_tag_add_success_test') do
    ...
    it 'drops all invalid attributes passed in' do
      VCR.use_cassette('simple_release_tag_add_success_test') do
        new_tag = '<release when="2016-01-05T23:23:45Z" who="petucket" to="Revs" what="self">true</release>'
        expect(item.identityMetadata.to_xml).not_to include new_tag
    ...
    it 'returns true when valid_release_attributes is called with valid attributes and no tag attribute' do
      @args.delete :tag
      expect(item.valid_release_attributes(true, @args)).to be true
    end
    it 'returns true when valid_release_attributes is called with valid attributes and tag attribute' do
      expect(item.valid_release_attributes(true, @args)).to be true
    end
    it 'raises an error when valid_release_attributes is called with a tag content that is not a boolean' do
