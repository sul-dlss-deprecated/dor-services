...
describe Dor::GeoMetadataDS do
  before do
    ...
    @test_keys.each do |k|
      @doc[k] = described_class.from_xml(read_fixture("geoMetadata_#{k}.xml"))
    end
  ...
  context 'Exports' do
    it '#to_bbox' do
      @test_keys.each do |k|
        expect(@doc[k]).to be_a(described_class)
    ...
    it '#xml_template' do
      expect(described_class.xml_template.to_xml).to be_equivalent_to(@template)
    end
