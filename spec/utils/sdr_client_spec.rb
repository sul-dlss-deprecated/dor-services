...
describe Sdr::Client do
  ...
  describe '.get_signature_catalog' do
    ...
    it 'fetches the signature catalog from SDR' do
      ...
      catalog = described_class.get_signature_catalog(druid)
  ...
  describe '.get_content_diff' do
    ...
    it 'fetches the file inventory difference from SDR' do
      ...
      inventory_difference = described_class.get_content_diff(druid, '')
  ...
  describe '.get_preserved_file_content' do
    ...
    it 'properly encodes filename and passes along the sdr response with the correct content type and status code' do
      resource = described_class.client["objects/#{druid}/content/#{URI.encode(filename_with_spaces)}?version=#{item_version}"]
      stub_request(:get, resource.url).to_return(body: sdr_resp_body, headers: { content_type: sdr_resp_content_type })
      expect(Deprecation).to receive(:warn)
      preserved_content = described_class.get_preserved_file_content(druid, filename_with_spaces, item_version)
    ...
    it 'passes errors through to the caller' do
      resource = described_class.client["objects/#{druid}/content/bogus_filename?version=#{item_version}"]
      stub_request(:get, resource.url).to_return(status: 404)
      expect(Deprecation).to receive(:warn)
      expect { described_class.get_preserved_file_content(druid, filename_with_spaces, item_version) }.to raise_error RestClient::ResourceNotFound
    end
  end
  ...
  describe '.client' do
    ...
    context 'with DOR services configuration' do
      ...
      it 'is configured to use SDR' do
        expect(Deprecation).to receive(:warn)
        expect(described_class.client.url).to eq Dor::Config.dor_services.rest_client['v1/sdr'].url
      end
    end
